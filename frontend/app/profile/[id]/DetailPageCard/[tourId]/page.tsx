...
const DetailPageCard = () => {
  const { id, tourId } = useParams(); // id is user ID, tourId is tour ID
  ...
  // Find the specific tour based on the tourId
  const tour = tours.find((tour) => tour._id === tourId);

  // Handle case where the tour is not found
  if (!tour && !loading) {
    return <div className="text-center py-10 font-semibold">Tour not found</div>;
  }
  ...
  // Handle real backend booking
  const handleBooking = async () => {
    if (quantity === 0) {
      alert("Please select at least 1 person to book.");
      return;
    }
    ...
    try {
      const bookingData = {
        tour: tour._id,
        company: tour.company?._id || tour.company,
        members: quantity,
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3500/api'}/bookings/${id}`,
        {
          method: "POST",
          headers: {
  ...
  const displayLocation = tour.location?.nameLocation || tour.location?.name || tour.location || "Location";
