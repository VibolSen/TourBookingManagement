...
import { useRouter, useParams } from 'next/navigation'; // Import the router for navigation
...
const Historical = () => {
        ...
        const fetchData = async () => {
            try {
                ...
                const filteredData = (data.data || [])
                    .filter((tour: Tour) => tour.category?.categoryname?.toLowerCase().includes("historical") || tour.category?.categoryname?.toLowerCase().includes("culture"))
                    .map((tour: Tour) => ({
                        id: tour._id,
                        image: tour.mainImage,
                        title: tour.name,
                        reviews: tour.reviews || 105,
                        price: `$${tour.price}`,
    ...
    const handleCardClick = (tourId: string) => {
        ...
        router.push(`/profile/${id}/DetailPageCard/${tourId}`);
    };
        ...
        <div className="px-4 sm:px-6 lg:px-20 py-8">
            ...
            {attractions.length > 0 ? (
                ...
                <div className="min-h-[30vh] flex flex-col items-center justify-center p-8 text-center bg-gray-50/50 rounded-2xl border border-gray-100 my-10 max-w-2xl mx-auto shadow-sm">
                    ...
                    <h3 className="text-2xl font-bold text-gray-800 mb-2">No Historical Places Found</h3>
                    <p className="text-gray-500 max-w-md">
                        There are currently no active historical packages in our database. Please check back later!
                    </p>
                </div>
            )}
        </div>
