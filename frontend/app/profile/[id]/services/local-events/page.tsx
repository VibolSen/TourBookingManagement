...
export default function LocalEvent() {
    ...
    // Fetch data from API (client-side only)
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const filteredData = (data.data || [])
                    .filter((tour: Tour) => tour.category?.categoryname?.toLowerCase().includes("event") || tour.category?.categoryname?.toLowerCase().includes("traditional"))
                    .map((tour: Tour) => ({
                        id: tour._id,
                        image: tour.mainImage,
                        title: tour.name,
                        reviews: tour.reviews || 88,
                        price: `$${tour.price}`,
    ...
    return (
        <div className="px-4 sm:px-6 lg:px-20 py-8">
            <h1 className="text-4xl font-bold p-2 mt-5 text-center my-4">
                We Offer the Best Popular Local Event Places
            </h1>
            {attractions.length > 0 ? (
                ...
                <div className="min-h-[30vh] flex flex-col items-center justify-center p-8 text-center bg-gray-50/50 rounded-2xl border border-gray-100 my-10 max-w-2xl mx-auto shadow-sm">
                    ...
                    <h3 className="text-2xl font-bold text-gray-800 mb-2">No Local Events Found</h3>
                    <p className="text-gray-500 max-w-md">
                        There are currently no active local event packages in our database. Please check back later!
                    </p>
                </div>
            )}
        </div>
