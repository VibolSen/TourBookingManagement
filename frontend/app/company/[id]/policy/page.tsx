...
const InviteCard = () => {
  ...
  const [selectedPolicyId, setSelectedPolicyId] = useState<string | null>(null); // State to track selected policy ID
  const [isActiveFilter, setIsActiveFilter] = useState<string | null>(null); // State for isActive filter
  const [editingPolicy, setEditingPolicy] = useState<Policy | null>(null); // State to track policy being edited
  const [showDeleteConfirmation, setShowDeleteConfirmation] = useState(false); // State for delete confirmation form
  const [policyToDelete, setPolicyToDelete] = useState<string | null>(null); // Track which policy is being deleted
  ...
  useEffect(() => {
    if (id) {
      fetchPolicies(id, isActiveFilter ? isActiveFilter === "true" : undefined);
    }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-2 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <div className="mb-4">
              ...
              {/* Render description as a numbered list */}
              <ol className="text-sm text-gray-600 list-decimal list-inside">
                {(policy.description as string).split("\n").map((item: string, index: number) => (
                  <li key={index}>{item}</li>
                ))}
              </ol>
            </div>
