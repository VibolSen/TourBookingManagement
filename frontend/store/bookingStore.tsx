...
interface BookingState {
  ...
  loading: boolean;
  error: string | null;
  fetchBookingStatisticsByCompanyId: (id: string | string[]) => Promise<void>;
  fetchBookings: () => Promise<void>;
  fetchBookingsBySubadmin: (subadminId: string | string[]) => Promise<void>;
  createBooking: (bookingData: any) => Promise<void>;
  updateBooking: (id: string, updatedData: any) => Promise<void>;
...
export const useBookingStore = create<BookingState>()((set) => ({
  ...
  createBooking: async (bookingData) => {
    set({ loading: true, error: null });
    try {
      const response = await fetch(`${API_URL}`, { // Note: using API_URL directly to prevent undefined userId crash
        method: "POST",
        headers: {
