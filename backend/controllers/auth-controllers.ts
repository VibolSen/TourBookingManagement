  ...
  try {
    ...
    res.status(200).json({
      ...
      user: {
        ...
        email: user.email,
        role: user.role,
        image: user.image && user.image.startsWith("http") ? user.image : (user.image ? `${process.env.API_URL}${path.basename(user.image)}` : null),
      },
    });
  } catch (error) {
    ...
    if (role && role !== previousRole && user.adminId) {
      ...
      if (admin) {
        if (previousRole === "user" && role === "subadmin") {
          ...
          if (typeof admin.userCount === "number") admin.userCount -= 1;
          if (typeof admin.subAdminCount === "number") admin.subAdminCount += 1;
...
export const companyUser = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      ...
      user: {
        ...user.toObject(),
        imageUrl: (user.toObject() as any).imageUrl || null, // Ensure imageUrl is included
      },
    });
  } catch (error) {
