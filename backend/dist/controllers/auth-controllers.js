...
exports.companyUser = exports.uploadImage = exports.deleteUser = exports.editUser = exports.getUsersByRole = exports.checkAuth = exports.resetPassword = exports.forgotPassword = exports.verifyEmail = exports.logout = exports.getProfile = exports.showGoogleAuth = exports.handleGoogle = exports.login = exports.signup = void 0;
const user_js_1 = require("../model/user.js");
const admin_js_1 = require("../model/admin.js");
const bcryptjs_1 = __importDefault(require("bcryptjs"));
const generateJWTToken_js_1 = require("../utils/generateJWTToken.js");
const generateVerificationToken_js_1 = require("../utils/generateVerificationToken.js");
const email_js_1 = require("../resend/email.js");
const crypto_1 = __importDefault(require("crypto"));
const path_1 = __importDefault(require("path"));
const axios_1 = __importDefault(require("axios"));
// Signup function
const signup = async (req, res) => {
    const { name, email, password, adminId, // Optional for users or subadmins
    status: requestedStatus, role: requestedRole, } = req.body;
    ...
    try {
        ...
        const userAlreadyExists = await user_js_1.User.findOne({ email });
        ...
        const verificationToken = (0, generateVerificationToken_js_1.generateVerificationToken)();
        // Create a new user
        ...
        if (role === "admin") {
            ...
            const admin = await admin_js_1.Admin.findById(allowedIDadmin);
            if (!admin) {
        ...
        (0, generateJWTToken_js_1.generateJWTToken)(res, user._id, user.role);
        await (0, email_js_1.sendVerificationEmail)(user.email, verificationToken);
    ...
    try {
        ...
        res.status(200).json({
            ...
            user: {
                ...
                email: user.email,
                role: user.role,
                image: user.image && user.image.startsWith("http") ? user.image : (user.image ? `${process.env.API_URL}${path_1.default.basename(user.image)}` : null),
            },
        });
    }
    catch (error) {
...
const handleGoogle = async (req, res) => {
    try {
        ...
        const userResponse = await axios_1.default.get(`https://www.googleapis.com/oauth2/v3/userinfo`, {
            headers: {
        ...
        let user = await user_js_1.User.findOne({ email: userInfo.email });
        if (!user) {
            ...
            if (user.role === "user") {
                ...
                return res.redirect(`${process.env.CLIENT_URL}/company/${user._id}/dashboard`);
            }
            // Redirect to the profile page with the user's ID
...
const showGoogleAuth = async (req, res) => {
...
exports.showGoogleAuth = showGoogleAuth;
// getprofile function
const getProfile = async (req, res) => {
    try {
        const user = await user_js_1.User.findById(req.userId); // Adjust the query as needed
        if (!user) {
...
const verifyEmail = async (req, res) => {
    ...
    try {
        ...
        await user.save();
        await (0, email_js_1.sendWelcomeEmail)(user.email, user.name);
        res
            .status(200)
...
const forgotPassword = async (req, res) => {
    ...
    try {
        ...
        await user.save();
        await (0, email_js_1.sendPasswordResetEmail)(user.email, `${process.env.CLIENT_URL}/reset-password/${resetPasswordToken}`);
        res.status(200).json({
            success: true,
...
const resetPassword = async (req, res) => {
    try {
        ...
        await user.save();
        await (0, email_js_1.sendResetSuccessEmail)(user.email);
        res
            .status(200)
...
const checkAuth = async (req, res) => {
    try {
        ...
        res
            .status(200)
            .json({ success: true, user: { ...user.toObject(), password: undefined } });
    }
    catch (error) {
...
const getUsersByRole = async (req, res) => {
    ...
    try {
        ...
        const users = await user_js_1.User.find(query).select("-password"); // Exclude password
...
const editUser = async (req, res) => {
    ...
    try {
        ...
        // Find the user first to track role changes
        ...
        user.role = role;
        await user.save();
        // Handle Admin lists sync if role changed and user is linked to an admin
        if (role && role !== previousRole && user.adminId) {
            const admin = await admin_js_1.Admin.findById(user.adminId);
            if (admin) {
                if (previousRole === "user" && role === "subadmin") {
                    // Move from users list to subAdmins list
                    admin.users = admin.users.filter((id) => id.toString() !== userId.toString());
                    if (!admin.subAdmins.map(id => id.toString()).includes(userId.toString())) {
                        admin.subAdmins.push(user._id);
                    }
                    if (typeof admin.userCount === "number")
                        admin.userCount -= 1;
                    if (typeof admin.subAdminCount === "number")
                        admin.subAdminCount += 1;
                }
                else if (previousRole === "subadmin" && role === "user") {
                    // Move from subAdmins list to users list
                    admin.subAdmins = admin.subAdmins.filter((id) => id.toString() !== userId.toString());
                    if (!admin.users.map(id => id.toString()).includes(userId.toString())) {
                        admin.users.push(user._id);
                    }
                    if (typeof admin.userCount === "number")
                        admin.userCount += 1;
        ...
        res.status(200).json({
            ...
            user: { ...user.toObject(), password: undefined }, // Exclude password
        });
    }
    catch (error) {
...
const deleteUser = async (req, res) => {
    ...
    try {
        ...
        const user = await user_js_1.User.findByIdAndDelete(userId);
        if (!user) {
            return res.status(404).json({
                success: false,
                message: "User not found",
...
const companyUser = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({
            ...
            user: {
                ...user.toObject(),
                imageUrl: user.toObject().imageUrl || null, // Ensure imageUrl is included
            },
        });
    }
    catch (error) {
